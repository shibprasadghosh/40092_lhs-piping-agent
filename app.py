...
if st.session_state.search_result_df is not None:
    ...
    if res_df.empty:
        ...
        # 1. CSV Download (সাথে ওয়াটারমার্ক রো থাকবে)
        csv_df = display_df.copy()
        csv_df.loc[len(csv_df)] = [""] * len(csv_df.columns)
        w_row = [""] * len(csv_df.columns)
        w_row[0] = "© Generated by LHS AI-Powered Dashboard - Created by Shib Prasad Ghosh"
        csv_df.loc[len(csv_df)] = w_row
        ...
        # 2. Excel Download (অফিশিয়াল ফুটার ওয়াটারমার্ক সহ - ডিলিট করা যাবে না!)
        try:
            excel_buffer = io.BytesIO()
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                display_df.to_excel(writer, index=False, sheet_name='Search_Result')
                
                # --- Excel Footer Watermark Setup ---
                worksheet = writer.sheets['Search_Result']
                worksheet.odd_footer.right.text = "© Created by Shib Prasad Ghosh"
                worksheet.even_footer.right.text = "© Created by Shib Prasad Ghosh"
